...
(function(mod) {
    ...
    CodeMirror.defineMode("smartymixed",
        function(config) {
            var settings,
                regs,
                helpers,
            ...
            parsers = {
                html: function(stream, state) {
                    if (!state.inLiteral &&
                        ...
                        return helpers.maybeBackup(stream,
                ...
                smarty: function(stream, state) {
                    if (stream.match(settings.leftDelimiter, false)) {
                        if (stream.match(regs.smartyComment, false)) {
                            return helpers.chain(stream,
                                state,
                                parsers.inBlock("comment", "*" + settings.rightDelimiter));
                        }
            ...
            return {
                ...
                copyState: function(state) {
                    ...
                    if (state.localState) {
                        local = CodeMirror.copyState((tok != parsers.html ? smartyMode : htmlMixedMode),
                            state.localState);
                    }
                    return {
                        token: state.token,
