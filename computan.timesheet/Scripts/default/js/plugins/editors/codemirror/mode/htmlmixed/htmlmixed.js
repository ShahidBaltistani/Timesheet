        ...
        function(config, parserConfig) {
            ...
            if (scriptTypesConf)
                for (var i = 0; i < scriptTypesConf.length; ++i) {
                    ...
                    scriptTypes.push(
                        { matches: conf.matches, mode: conf.mode && CodeMirror.getMode(config, conf.mode) });
                }
            scriptTypes.push({
                matches: /./,
            ...
            function html(stream, state) {
                ...
                if (tagName == "script" && /\btag\b/.test(style) && stream.current() == ">") {
                    ...
                    var scriptType = stream.string.slice(Math.max(0, stream.pos - 100), stream.pos)
                        .match(/\btype\s*=\s*("[^"]+"|'[^']+'|\S+)[^<]*$/i);
                    ...
                    for (var i = 0; i < scriptTypes.length; ++i) {
                        ...
                        if (typeof tp.matches == "string" ? scriptType == tp.matches : tp.matches.test(scriptType)) {
                            if (tp.mode) {
                                ...
                                state.localState =
                                    tp.mode.startState && tp.mode.startState(htmlMode.indent(state.htmlState, ""));
                            }
                            break;
                        }
                    }
