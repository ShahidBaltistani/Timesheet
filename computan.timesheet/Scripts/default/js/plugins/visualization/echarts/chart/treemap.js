define("echarts/chart/treemap",
    [
        "require", "./base", "zrender/tool/area", "zrender/shape/Rectangle", "zrender/shape/Text", "zrender/shape/Line",
        "../layout/TreeMap", "../data/Tree", "../config", "../util/ecData", "zrender/config", "zrender/tool/event",
    ...
    function(e) {
        function t(e, t, n, a, o) {
            ...
            var r = this;
            r._onclick = function(e) { return r.__onclick(e) }, r.zr.on(V.EVENT.CLICK, r._onclick);
        }
        ...
        return t.prototype = {
            ...
            _buildSeries: function(e, t) {
                ...
                this._treesMap[t] = i, this._buildTreemap(i.root, t);
            },
            _buildTreemap: function(e, t) {
                ...
                for (var o = i.length,
                    r = this.series[t],
                    l = r.itemStyle,
                    h = this.parsePercent(r.size[0], this.zr.getWidth()) || 400,
                    V = this.parsePercent(r.size[1], this.zr.getHeight()) || 500,
                    U = this.parseCenter(this.zr, r.center),
                    d = U[0] - .5 * h,
                    p = U[1] - .5 * V,
                    c = h * V,
                    u = 0,
                ...
                for (var f = new s({ x: d, y: p, width: h, height: V }), k = f.run(y), x = 0; x < k.length; x++) {
                    var _ = g[x].data, L = k[x], W = [_.itemStyle, l], X = this.deepMerge(W);
                    X.normal.color || (X.normal.color = this.zr.getColor(x)),
                        X.emphasis.color || (X.emphasis.color = X.normal.color), this._buildItem(_, X, L, t, x),
                        _.children && this._buildChildrenTreemap(_.children, X, L, t);
                }
                ...
                var n = t.emphasis,
                    ...
                    l = {
                        ...
                        highlightStyle: U.merge(
                            { color: n.color, lineWidth: n.borderWidth, strokeColor: n.borderColor },
                            r.highlightStyle,
                            !0)
                    };
                return new a(l);
            ...
            getLabel: function(e, t, i, a) {
                ...
                e.normal.label.show
                    ? (e.normal.label.x + V > t.width || e.normal.label.y + U > t.height) && (h = "")
                    : h = "", e.emphasis.label.show ? (s.x + u > t.width || s.y + y > t.height) && (p = "") : p = "";
                var g = {
                    style: {
            ...
            getLabelText: function(e, t, i) {
                return i
                    ? "function" == typeof i
                    ? i.call(this.myChart, e, t)
                    : "string" == typeof i
                    ? (i = i.replace("{b}", "{b0}").replace("{c}", "{c0}"), i = i.replace("{b0}", e).replace("{c0}", t))
                    : void 0
                    : e;
            },
            _buildChildrenTreemap: function(e, t, i, n) {
                ...
                for (var V = new s({ x: i.x, y: i.y, width: i.width, height: i.height }),
                    U = V.run(r),
                    d = t.normal.childBorderWidth || 1,
                    p = t.normal.childBorderColor || "#777",
                    c = 0;
                    c < U.length;
                    c++) {
                    var u = U[c], y = [];
                    i.y.toFixed(2) !== u.y.toFixed(2) && y.push(this._getLine(u.x, u.y, u.x + u.width, u.y, d, p)),
                        i.x.toFixed(2) !== u.x.toFixed(2) && y.push(this._getLine(u.x, u.y, u.x, u.y + u.height, d, p)),
                        (i.y + i.height).toFixed(2) !== (u.y + u.height).toFixed(2) &&
                            y.push(this._getLine(u.x, u.y + u.height, u.x + u.width, u.y + u.height, d, p)),
                        (i.x + i.width).toFixed(2) !== (u.x + u.width).toFixed(2) &&
                            y.push(this._getLine(u.x + u.width, u.y, u.x + u.width, u.y + u.height, d, p));
            ...
            _buildBreadcrumb: function(e, t, i, n) {
                ...
                for (var s = this.series[t],
                    l = this.query(s, "itemStyle.normal.breadcrumb.textStyle") || {},
                    ...
                    p = { brushType: "fill", color: h.color || d.lift(l.color, -.3), textFont: this.getFont(h) },
                    c = 0;
                    c < a.length;
                    c++) {
                    var u = new o({
                    ...
                    m.set(u, "seriesIndex", t), m.set(u, "name", a[c]), i += u.getRect(u.style).width, this.shapeList
                        .push(u);
                }
            },
        ...
        return e.prototype.run = function(e) {
            ...
            if ("VERTICAL" == n) {
                for (var m = 0; m < r.length; m++)
                    i.push({ x: r[m].x + t.x, y: r[m].y + t.y, width: r[m].width, height: r[m].height });
                h = { x: r[0].width + t.x, y: t.y, width: t.width - r[0].width, height: t.height };
            } else {
                for (var V = 0; V < r.length; V++)
                    i.push({ x: r[V].y + t.x, y: r[V].x + t.y, width: r[V].height, height: r[V].width });
