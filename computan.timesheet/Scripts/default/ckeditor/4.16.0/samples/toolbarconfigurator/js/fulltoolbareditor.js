...
(function() {
    ...
    e.prototype.init = function(b) {
        ...
        this.editorInstance.once("configLoaded",
            function(d) {
                var c =
                    d.editor.config;
    ...
    e.prototype.groupButtonNamesByGroup = function(b) {
        ...
        for (var d in b)
            b[d] =
                e.map(b[d], function(b) { return a.getCamelCasedButtonName(b.name) });
        return b;
    };
    e.prototype.getGroupByName = function(b) {
        for (var a = this.editorInstance.config.toolbarGroups || this.getFullToolbarGroupsConfig(), d = a.length, c = 0;
            c < d;
            c += 1) if (a[c].name === b) return a[c];
        return null;
    };
