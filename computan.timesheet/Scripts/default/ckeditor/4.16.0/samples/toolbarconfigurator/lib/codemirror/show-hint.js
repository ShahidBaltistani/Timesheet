(function(f) {
    ...
    function w(a, b) {
        function c(a,
            c) {
            var d;
            d = "string" != typeof c ? function(a) { return c(a, b) } : e.hasOwnProperty(c) ? e[c] : c;
            f[a] = d;
        }

        var e = {
                Up: function() { b.moveFocus(-1) },
                Down: function() { b.moveFocus(1) },
                PageUp: function() { b.moveFocus(-b.menuSize() + 1, !0) },
                PageDown: function() { b.moveFocus(b.menuSize() - 1, !0) },
                Home: function() { b.setFocus(0) },
                End: function() { b.setFocus(b.length - 1) },
                Enter: b.pick,
                Tab: b.pick,
                Esc: b.close
            },
    ...
    function n(a, b) {
        ...
        if (0 < h.bottom - k) {
            ...
            0 < g.top - (g.bottom - h.top) - u
                ...
                : u > k &&
                (d.style.height = k - 5 + "px", d.style.top = (t = g.bottom - h.top) + "px", k =
                    e.getCursor(), b.from.ch != k.ch &&
                    (g = e.cursorCoords(k), d.style.left = (r = g.left) + "px", h = d.getBoundingClientRect()));
        }
        k = h.right - l;
        0 < k &&
        (h.right - h.left > l && (d.style.width = l - 5 + "px", k -= h.right - h.left - l), d.style.left =
            (r = g.left - k) + "px");
        e.addKeyMap(this.keyMap = w(a,
            {
                ...
                setFocus: function(a) { c.changeActive(a) },
                menuSize: function() { return c.screenAmount() },
                length: m.length,
                close: function() { a.close() },
                pick: function() { c.pick() },
                data: b
            }));
        if (a.options.closeOnUnfocus) {
            var p;
            e.on("blur", this.onBlur = function() { p = setTimeout(function() { a.close() }, 100) });
            e.on("focus", this.onFocus = function() { clearTimeout(p) });
        }
        ...
        f.on(d, "mousedown", function() { setTimeout(function() { e.focus() }, 20) });
    ...
    f.defineExtension("showHint",
        function(a) {
            1 < this.listSelections().length ||
                this.somethingSelected() ||
                (this.state.completionActive && this.state.completionActive.close(), a = this.state.completionActive =
                    new p(this, a), a.options.hint && (f.signal(this, "startCompletion", this), a.update()));
        });
    ...
    p.prototype = {
        close: function() {
            this.active() &&
            (this.tick = this.cm.state.completionActive =
                null, this.cm.off("cursorActivity", this.activityFunc), this.widget && this.widget.close(), f.signal(
                this.cm,
                "endCompletion",
                this.cm));
        },
        ...
        pick: function(a, b) {
            ...
            c.hint
                ? c.hint(this.cm, a, c)
                : this.cm.replaceRange("string" == typeof c ? c : c.text,
                    c.from || a.from,
                    c.to || a.to,
                    "complete");
            f.signal(a, "pick", c);
        ...
        cursorActivity: function() {
            ...
            if (a.line != this.startPos.line ||
                b.length - a.ch != this.startLen - this.startPos.ch ||
                a.ch < this.startPos.ch ||
                this.cm.somethingSelected() ||
                a.ch &&
                this.options.closeCharacters.test(b.charAt(a.ch -
                    1))) this.close();
            else {
                var c = this;
                this.debounce = x(function() { c.update() });
        ...
        update: function() {
            if (null != this.tick)
                if (this.data && f.signal(this.data, "update"), this.options.hint.async) {
                    ...
                    this.options.hint(this.cm, function(c) { b.tick == a && b.finishUpdate(c) }, this.options);
    ...
    n.prototype = {
        ...
        disable: function() {
            this.completion.cm.removeKeyMap(this.keyMap);
            var a = this;
            this.keyMap = { Enter: function() { a.picked = !0 } };
        ...
        changeActive: function(a, b) {
            ...
            if (this.selectedHint != a) {
                ...
                c.offsetTop < this.hints.scrollTop
                    ...
                    : c.offsetTop + c.offsetHeight > this.hints.scrollTop + this.hints.clientHeight &&
                    (this.hints.scrollTop = c.offsetTop + c.offsetHeight - this.hints.clientHeight + 3);
