...
(function() {
    ...
    function t(a) {
        return function() {
            var f = this.getValue(), f = !!(CKEDITOR.dialog.validate.integer().call(this, f) && 0 < f);
    ...
    function r(a, f) {
        var p = function(d) { return new CKEDITOR.dom.element(d, a.document) },
            r = a.editable(),
        ...
        return{
            ...
            onLoad: function() {
                ...
                if (a)
                    a.on("change",
                        function() {
                            var a = this.getStyle("width", ""), c = d.getContentElement("info", "txtWidth");
                            c && c.setValue(a, !0);
            ...
            onShow: function() {
                ...
                ((d = d.getSelectedElement()) && d.is("table")
                    ? b = d
                    : 0 < e.length &&
                    (CKEDITOR.env.webkit && e[0].shrink(CKEDITOR.NODE_ELEMENT), b =
                        a.elementPath(e[0].getCommonAncestor(!0)).contains("table", 1)), this._.selectedElement = b);
            ...
            onOk: function() {
                ...
                if (c.info) {
                    ...
                    if (!this._.selectedElement)
                        for (var h = b.append(p("tbody")),
                            f = parseInt(c.txtRows, 10) || 0,
                            l = parseInt(c.txtCols, 10) || 0,
                            k = 0;
                            k < f;
                            k++) for (var g = h.append(p("tr")), m = 0; m < l; m++) g.append(p("td")).appendBogus();
                    f = c.selHeaders;
                    if (!b.$.tHead && ("row" == f || "both" == f)) {
                        ...
                        for (k = 0; k < l.getChildCount(); k++)
                            h = l.getChild(k), h.type != CKEDITOR.NODE_ELEMENT ||
                                h.data("cke-bookmark") ||
                    ...
                    if (null !== b.$.tHead && "row" != f && "both" != f) {
                        ...
                        for (h = b.getElementsByTag("tbody").getItem(0); 0 < g.getChildCount();) {
                            l = g.getFirst();
                            for (k = 0; k < l.getChildCount(); k++)
                                m = l.getChild(k), m.type == CKEDITOR.NODE_ELEMENT &&
                    ...
                    if (!this.hasColumnHeaders && ("col" == f || "both" == f))
                        for (g = 0; g < b.$.rows.length; g++)
                            m = new CKEDITOR.dom.element(b.$.rows[g].cells[0]), m.renameNode("th"), m.setAttribute(
                                "scope",
                                "row");
                    if (this.hasColumnHeaders && "col" != f && "both" != f)
                        for (k = 0; k < b.$.rows.length; k++)
                            g = new CKEDITOR.dom.element(b.$.rows[k]), "tbody" == g.getParent().getName() &&
                            (m = new CKEDITOR.dom.element(g.$.cells[0]), m.renameNode("td"), m.removeAttribute(
                                "scope"));
                ...
                if (this._.selectedElement)
                    ...
                    a.insertElement(b), setTimeout(function() {
                            var d = new CKEDITOR.dom.element(b.$.rows[0].cells[0]), c = a.createRange();
                                    ...
                                    children: [
                                        {
                                            ...
                                            setup: function(d) { this.setValue(d.$.rows.length) },
                                            commit: n
                                        },
                                        {
                                            type: "text",
                                            ...
                                            setup: function(d) { this.setValue(w(d)) },
                                            commit: n
                                        ...
                                        },
                                        {
                                            ...
                                            setup: function(d) { this.setValue(d.getAttribute("border") || "") },
                                            commit: function(d, a) {
                                                this.getValue()
                                            ...
                                            items: [
                                                ...
                                                [a.lang.common.center, "center"], [a.lang.common.right, "right"]
                                            ],
                                        ...
                                        if (this.isEnabled()) {
                                            ...
                                            if (b)
                                                0 < c.count()
                                                    ...
                                                    : (c = new CKEDITOR.dom.element("caption", a.document), e.append(c,
                                                        !0)), c.append(new CKEDITOR.dom.text(b, a.document));
                                            else if (0 < c.count())
                                                for (b = c.count() -
                                                        1;
                                                    0 <= b;
                                                    b--) c.getItem(b).remove();
                                        }
                                    }
                                },
                                {
