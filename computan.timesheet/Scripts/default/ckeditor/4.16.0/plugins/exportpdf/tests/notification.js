        ...
        function() {
            ...
            bender.test({
                    ...
                    editor.once("exportPdf",
                        function() {
                            assertNotifications(editor,
                                { message: processingDocumentMsg, type: "progress", progress: 0 });
                        });

                    editor.once("exportPdf",
                        function() {
                            assertNotifications(editor,
                                { message: processingDocumentMsg, type: "progress", progress: 0.2 });
                        },
                        null,
                        null,
                        16);

                    exportPdfUtils.useXHR(editor,
                        function(xhr) {
                            xhr.addEventListener("progress",
                                function() {
                                    assertNotifications(editor,
                                        { message: processingDocumentMsg, type: "progress", progress: 0.8 });
                                });

                            xhr.addEventListener("loadend",
                                function() {
                                    assertNotifications(editor,
                                        { message: "Document is ready!", type: "success", progress: 1 });
                                });
                ...
                'test notifications and progress steps are correct in sad path': function() {
                    ...
                    exportPdfUtils.useXHR(editor,
                        function(xhr) {
                            var stubbed = sinon.stub(console,
                                "error",
                                function(arg) {
                                    assert.areSame("Validation failed.",
                                        arg.message,
            ...
            function assertNotifications(editor, expectedNotification) {
                ...
                assert.areSame(expectedNotification.progress,
                    actualNotification.progress,
