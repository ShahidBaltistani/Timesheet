        ...
        function() {
            ...
            bender.test({
                ...
                'test token is fetched if tokenUrl is correct': function() {
                    createEditor({ exportPdf_tokenUrl: "/custom-url" },
                        function(editor) {
                            editor.on("exportPdf",
                                function(evt) {
                ...
                'test authentication header is added if token is provided': function() {
                    createEditor({ exportPdf_tokenUrl: "/custom-url" },
                        function(editor) {
                            ...
                            assert.areEqual(tokenValue,
                                xhrServer.requests[xhrServer.requests.length - 1].requestHeaders["Authorization"],
                ...
                'test console.warn is called if tokenUrl is not provided': function() {
                    CKEDITOR.once("log",
                        function(evt) {
                            ...
                            assert.areEqual("exportpdf-no-token-url",
                                evt.data.errorCode,
                ...
                'test console.warn is called on POST request if token is empty': function() {
                    ...
                    createEditor({ exportPdf_tokenUrl: "/empty-token" },
                        function(editor) {
                            xhrServer.respond();
                            editor.execCommand("exportPdf");
                            xhrServer.respond();
                ...
                'test token refreshes in the declared intervals': function() {
                    CKEDITOR.once("instanceCreated",
                        function(evt) {
                            evt.editor.exportPdfTokenInterval = 200;
                        });

                    createEditor({ exportPdf_tokenUrl: "/incremental_token" },
                        function(editor) {
                            xhrServer.respond();
                            setTimeout(function() {
                                    resume(function() {
                                        ...
                                        editor.on("exportPdf",
                                            function(evt) {
                                                assert.areNotSame(evt.data.token,
                                                    tokenValue + "0",
