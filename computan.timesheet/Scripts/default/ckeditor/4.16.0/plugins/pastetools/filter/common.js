...
(function() {
    ...
    function r(a) { return/%$/.test(a) ? a : a + "px" }

    function t(a) {
        var b = a.margin ? "margin" : a.MARGIN ? "MARGIN" : !1, c, d;
        if (b) {
    ...
    function u(a) {
        var b =
                "background-color:transparent;background:transparent;background-color:none;background:none;background-position:initial initial;background-repeat:initial initial;caret-color;font-family:-webkit-standard;font-variant-caps;letter-spacing:normal;orphans;widows;text-transform:none;word-spacing:0px;-webkit-text-size-adjust:auto;-webkit-text-stroke-width:0px;text-indent:0px;margin-bottom:0in"
                    .split(";"),
            c = CKEDITOR.tools.parseCssText(a.attributes.style),
            d,
            e;
        for (d in c)
            e = d + ":" + c[d], CKEDITOR.tools.array.some(b,
                    function(a) { return e.substring(0, a.length).toLowerCase() === a }) &&
                delete c[d];
    ...
    function v(a) {
        ...
        b = CKEDITOR.tools.array.map(a.split(";"), function(a) { return-1 === a.indexOf("/") ? a : a.split("/")[1] });
        return b.length ? b : !1;
    }

    function w(a, b) {
        var c = a.split(",");
        return CKEDITOR.tools.array.find(b,
                function(a) {
            ...
            elements: {
                "^": function(a) {
                    ...
                    if (a.attributes.bgcolor) {
                        ...
                        b["background-color"] ||
                        (b["background-color"] = a.attributes.bgcolor, a.attributes.style =
                            CKEDITOR.tools.writeCssText(b));
                    }
                },
                span: function(a) { if (a.hasClass("Apple-converted-space")) return new CKEDITOR.htmlParser.text(" ") },
                table: function(a) {
                    a.filterChildren(c);
                    var b = a.parent, f = b && b.parent, d, h;
                    if (b.name &&
                        "div" === b.name &&
                        b.attributes.align &&
                ...
                tr: function(a) { a.attributes = {} },
                td: function(a) {
                    var d = a.getAscendant("table"), d = m.parseCssText(d.attributes.style, !0), f = d.background;
                    f &&
                        g.setStyle(a,
                            "background",
                            f,
                    ...
                    d.background &&
                    (h = CKEDITOR.tools.style.parse.background(d.background), h.color &&
                        (g.setStyle(a, "background-color", h.color, !0), g.setStyle(a, "background", "")));
                    for (var n in f)
                        h =
                            d[n] ? CKEDITOR.tools.style.border.fromCssRule(d[n]) : f[n], "none" === h.style
                            ? g.setStyle(a, n, "none")
                            : g.setStyle(a, n, h.toString());
                    g.mapCommonStyles(a);
                    g.convertStyleToPx(a);
                    g.createStyleStack(a,
                        c,
                        b,
                        /margin|text\-align|padding|list\-style\-type|width|height|border|white\-space|vertical\-align|background/i);
                },
                font: function(a) { a.attributes.face && d && (a.attributes.face = w(a.attributes.face, d)) }
            }
        };
    };
    p.styles = {
        setStyle: function(a, b, c, d) {
            ...
            ("" === c
                ? delete e[b]
                : e[b] =
                c, a.attributes.style = CKEDITOR.tools.writeCssText(e));
        },
        convertStyleToPx: function(a) {
            var b = a.attributes.style;
            b &&
            (a.attributes.style = b.replace(/\d+(\.\d+)?pt/g,
                function(a) { return CKEDITOR.tools.convertToPx(a) + "px" }));
        },
        ...
        mapCommonStyles: function(a) {
            return g.mapStyles(a,
                {
                    vAlign: function(b) { g.setStyle(a, "vertical-align", b) },
                    width: function(b) {
                        g.setStyle(a,
                            "width",
                            r(b));
                    },
                    height: function(b) { g.setStyle(a, "height", r(b)) }
                });
        },
        normalizedStyles: function(a, b) {
            var c =
                    "background-color:transparent border-image:none color:windowtext direction:ltr mso- visibility:visible div:border:none"
                        .split(" "),
                ...
                k = !0 ===
                    CKEDITOR.plugins.pastetools.getConfigValue(b,
                        "removeFontStyles"),
                f = m.parseCssText(a.attributes.style);
            "cke:li" == a.name &&
            (f["TEXT-INDENT"] && f.MARGIN
                ? (a.attributes["cke-indentation"] = p.lists.getElementIndentation(a), f.MARGIN =
                    f.MARGIN.replace(/(([\w\.]+ ){3,3})[\d\.]+(\w+$)/, "$10$3"))
                : delete f["TEXT-INDENT"], delete f["text-indent"]);
            for (var l = m.object.keys(f), h = 0; h < l.length; h++) {
                var n = l[h].toLowerCase(), g = f[l[h]], q = CKEDITOR.tools.indexOf;
            ...
            t(f);
            (function() {
                CKEDITOR.tools.array.forEach(["top", "right", "bottom", "left"],
                    function(a) {
                        a = "margin-" + a;
                        if (a in f) {
        ...
        createStyleStack: function(a, b, c, d) {
            ...
            for (f in b)
                if (!f.match(
                    d || /margin((?!-)|-left|-top|-bottom|-right)|text-indent|text-align|width|border|padding/i))
                    if (k) k = !1;
                    else {
        ...
        sortStyles: function(a) {
            for (var b = [
                ...
                f = 0;
                f < d.length;
                f++) -1 !== m.indexOf(b, d[f].toLowerCase()) ? e.push(d[f]) : k.push(d[f]);
            e.sort(function(a, c) {
                var d = m.indexOf(b, a.toLowerCase()), e = m.indexOf(b, c.toLowerCase());
                return d - e;
            });
            d = [].concat(e, k);
            e = {};
        ...
        inliner: {
            ...
            parse: function(a) {
                function b(a) {
                    var b = new CKEDITOR.dom.element("style"), c = new CKEDITOR.dom.element("iframe");
                    c.hide();
                ...
                var d = CKEDITOR.tools.parseCssText, e = g.inliner.filter, k = a.is ? a.$.sheet : b(a);
                a = [];
                var f;
                if (k)
            ...
            filter: function(a) {
                var b = g.inliner.filtered, c = m.array.indexOf, d = {}, e;
                for (e in a) -1 === c(b, e) && (d[e] = a[e]);
                return d;
            },
            sort: function(a) {
                return a.sort(function(a) {
                    var c = CKEDITOR.tools.array.map(a, function(a) { return a.selector });
                    return function(a, b) {
            ...
            inline: function(a) {
                ...
                CKEDITOR.tools.array.forEach(c,
                    function(a) {
                        ...
                        t(b);
                        for (h = 0; h < a.count(); h++)
                            c = a.getItem(h), g = CKEDITOR.tools.parseCssText(c.getAttribute("style")), t(g), g =
