...
(function() {
    function u(a, c, b) {
        ...
        e = CKEDITOR.tools.array.map(c, function(a) { return h(a) }, this);
    ...
    function v(a,
        c,
        b) {
        var e = CKEDITOR.tools.array.unique(CKEDITOR.tools.array.filter(b, function(a) { return a.match(/^blob:/i) }));
        b = CKEDITOR.tools.array.map(e, m);
        CKEDITOR.tools.promise.all(b).then(function(b) {
            CKEDITOR.tools.array.forEach(b,
                function(b, c) {
                    if (b) {
                        var d = e[c], d = a.editable().find('img[src\x3d"' + d + '"]').toArray();
    ...
    function l(a) {
        function c(a) {
            return"string" !== typeof a ? -1 : CKEDITOR.tools.array.indexOf(d, function(b) { return b.id === a });
        }
        ...
        var e = CKEDITOR.plugins.pastetools.filters.common.rtf, d = [];
        ...
        for (var k = 0; k < a.length; k++) {
            ...
            l
                ...
                : p ||
                m ||
                (g = { id: h, hex: q ? e.extractGroupContent(g).replace(/\s/g, "") : null, type: n }, -1 !== f
                    ? d.splice(f, 1, g)
                    : d.push(g));
        }
        return d;
    ...
    function r(a) {
        var c = CKEDITOR.tools.array.find(CKEDITOR.pasteFilters.image.recognizableImageTypes,
            function(b) { return b.marker.test(a) });
        return c ? c.type : "unknown";
    }

    function h(a) {
        var c = -1 !== CKEDITOR.tools.array.indexOf(CKEDITOR.pasteFilters.image.supportedImageTypes, a.type), b = a.hex;
        if (!c) return null;
    ...
    function t(a) {
        a = a.subarray(0,
            4);
        var c = CKEDITOR.tools.array.map(a, function(a) { return a.toString(16) }).join("");
        return(a = CKEDITOR.tools.array.find(CKEDITOR.pasteFilters.image.recognizableImageSignatures,
                function(a) { return 0 === c.indexOf(a.signature) }))
            ? a.type
            : null;
    }
    ...
    CKEDITOR.pasteFilters.image.getImageType =
        r;
    ...
    CKEDITOR.pasteFilters.image.recognizableImageTypes = [
        { marker: /\\pngblip/, type: "image/png" }, { marker: /\\jpegblip/, type: "image/jpeg" },
        { marker: /\\emfblip/, type: "image/emf" }, { marker: /\\wmetafile\d/, type: "image/wmf" }
    ];
    CKEDITOR.pasteFilters.image.recognizableImageSignatures =
    [
        { signature: "ffd8ff", type: "image/jpeg" }, { signature: "47494638", type: "image/gif" },
