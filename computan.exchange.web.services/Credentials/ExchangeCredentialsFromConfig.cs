...
{
    public class ExchangeCredentialsFromConfig : IExchangeCredentials
    {
        public static ExchangeCredentialsFromConfig ExchangeCredentials;

        public static IExchangeCredentials GetExchangeCredentials()
        {
            if (ExchangeCredentials == null)
            {
                GetExchangeCredentialsFromConfig();
            }
            return ExchangeCredentials;
        }

        public static IExchangeCredentials GetExchangeCredentials(string Email, string Password)
        {
            try
            {
                ...
                if (ExchangeCredentials == null)
                {
                    ExchangeCredentials = new ExchangeCredentialsFromConfig
                    {
        ...
        private static void GetExchangeCredentialsFromConfig()
        {
            ExchangeCredentials = new ExchangeCredentialsFromConfig();

            // Fetch Email Address.
            if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["EmailAddress"]))
            {
                ExchangeCredentials.EmailAddress = ConfigurationManager.AppSettings["EmailAddress"];
            }
            else
            {
                ExchangeCredentials.EmailAddress = "computansupport";
            }

            // Fetch Password.           
            string Password = "computan#!@";
            if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["Password"]))
            {
                Password = ConfigurationManager.AppSettings["Password"];
            }
            ExchangeCredentials.Password = new SecureString();
            foreach (char c in Password)
            {
                ExchangeCredentials.Password.AppendChar(c);
            }
